...
require_relative 'ZefoyTiktokBot_types'


class ZefoyTiktokBotSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ZefoyTiktokBotHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ZefoyTiktokBotFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ZefoyTiktokBotUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ZefoyTiktokBotError.new(
        "#{op}_allow",
        "ZefoyTiktokBotSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ZefoyTiktokBotError => err
    ...
    if fetched.is_a?(Hash)
      status = ZefoyTiktokBotHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ZefoyTiktokBotError.new(
        "graphql_error", "ZefoyTiktokBotSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ZefoyTiktokBotSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
