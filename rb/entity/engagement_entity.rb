...
class EngagementEntity
  ...
  def data_set(args)
    if args
      @_data = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Engagement, Hash] the created Engagement; raises ZefoyTiktokBotError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = ZefoyTiktokBotHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
