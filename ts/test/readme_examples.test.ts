...
//     (\`new ZefoyTiktokBotSDK(...)\` / \`ZefoyTiktokBotSDK.test(...)\` become
//     \`ZefoyTiktokBotSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ZefoyTiktokBotSDK'
...
// rewrite real client construction (\`new ZefoyTiktokBotSDK(...)\` and any
// \`ZefoyTiktokBotSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ZefoyTiktokBotSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ZefoyTiktokBotSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
